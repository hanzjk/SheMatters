...
$profile = mysqli_query($link, "SELECT police_name FROM police_users WHERE police_id ='" . $row['police_id'] . "'");
...
<?php if ($row['state'] == 'Ongoing'|| $row['state'] == 'Ongoing_Currently' || $row['state'] == 'Closed') { ?>
    <span class="btn btn-info "> Transfered Police Station &nbsp;<i class="fa fa-arrow-right" aria-hidden="true"></i> &nbsp; <?php echo $rowp['police_name']; ?>
    </span>
    <?php } ?>
</div>
...
<div class="row mt-2 ">
    ...
    <?php if ($row['state'] == 'Ongoing') { ?>
        <span class="btn btn-danger "> Still not confirmed the receipt of the complaint.
        </span>
    <?php } ?>


    <?php if ($row['state'] == 'Ongoing_Currently') {
        $date = mysqli_query($link, "SELECT confirmed_date FROM police_complaints WHERE complaint_id ='" . $complaint_id . "'");
        $drow = mysqli_fetch_assoc($date);
        ?>
        <span class="btn btn-success "> Confirmed the receipt of the complaint on <?php echo $drow['confirmed_date'] ?>
        </span>
    <?php } ?>
   
    
    <?php if ($row['state'] == 'Closed') {
        $date = mysqli_query($link, "SELECT closed_date FROM police_complaints WHERE complaint_id ='" . $complaint_id . "'");
        $drow = mysqli_fetch_assoc($date);
        ?>
        <span class="btn btn-success "> Closed the complaint on <?php echo $drow['closed_date'] ?>
        </span>
    <?php } ?>
</div>
